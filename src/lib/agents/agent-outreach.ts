...
export class Aventrautreach extends AgentRunner {
  async run(input: AgentInput): Promise<AgentResult> {
    const { lead_id, team_id } = input
    if (!lead_id) {
      return { success: false, agent: 'Aventrautreach', action_taken: 'none', error: 'MISSING_LEAD_ID' }
    }
    ...
    if (fetchError || !lead) {
      return { success: false, agent: 'Aventrautreach', action_taken: 'none', error: 'LEAD_NOT_FOUND' }
    }
    ...
    if (queueError) {
      console.error('[Aventrautreach] Queue insert failed:', queueError)
    }
